...
5. Yeni bir `VAPI_WEBHOOK_SECRET` üret, `apps/crm` env'ine yaz. Vapi sunucu ayarlarında özel header desteği varsa `x-vapi-webhook-secret: <yeni-secret>` header'ını tercih et (secret log'larda düz metin kalmaz); desteklemiyorsa `vapi-telesekreter/assistant.json`'daki `server.url`'i `https://<crm-domain>/api/webhooks/vapi?token=<yeni-secret>` olarak güncelle — bu fallback hâlâ çalışıyor (ayrı repo, bu depo dışı bir işlem).
